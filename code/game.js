...
var actorChars = {
  ...
  //"=": Lava, "|": Lava, "v": Lava
  //Bullet was my new hazard but if I comment it out I get the 'runGame'
  //not defined error!
  //"=" : Bullet, "<" Bullet, //= goes right, < goes left
  "v" : Lava,
  "w" : Wings
};
...
function Wings(pos, ch){
  this.pos = pos;
  this.size = new Vector
  //make it wibble
  thiswibble = Math.random()*Math.PI*2;
}
...
Bullet.prototype.act = function(step, level){
  ...
  if (!level.obstacleAt(newPos, this.size))
    ...
    this.post = this.repeatPos;
  else
    this.speed = this.speed.times(-2.5);
}
...
var wibbleSpeed = 9, wibbleDist = 0.09;

Wings.prototype.act = function(step){
  this.wibble += step*wibbleSpeed;
  var wibblePos = Math.sin(this.wibble) * wibbleDist;
  this.pos = this.basePose.plus(new Vector (0, wibblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
   ...
   deathsound.play()
    this.finishDelay = 1;
  } else if (type == "bullet" && this.status == null){
    this.status = "lost";
    deathsound.play()
  }else if (type == "wings"){
    //invulnerability maybe
    this.status = "glow";
    var wingsound = new Audio('argitoth-gates-of-heaven.mp3')
    wingsound.play()
    //make the invulnerability have a time limit
    var timer = setInterval(myTimer, 3000);
    function myTimer(){
      this.status=null;
    }
  }else if (type == "coin") {
    var coinsound = new Audio('timgormly__8-bit-coin.mp3')
    coinsound.play()
...
function runGame(plans, Display) {
  function startLevel(n) {
    ...
    //happy bark bc doggie is excited to play!
    var barkSound = new Audio('cgeffex__happyshortbark.mp3')
      barkSound.play()
